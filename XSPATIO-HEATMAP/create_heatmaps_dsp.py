...
def parse_config_dict(cli_args, config_dict):
    if cli_args.save_exp_code is not None:
        config_dict['exp_arguments']['save_exp_code'] = cli_args.save_exp_code
    if cli_args.overlap is not None:
        config_dict['patching_arguments']['overlap'] = cli_args.overlap
    return config_dict
...
if __name__ == '__main__':
    ...
    def_seg_params = {'seg_level': 0, 'sthresh': 0.8, 'mthresh': 3, 'close': 5, 'use_otsu': True,
                      'keep_ids': 'none', 'exclude_ids': 'none'}
    def_filter_params = {'a_t': 0.2, 'a_h': 0.05, 'max_n_holes': 10}
    def_vis_params = {'vis_level': 0, 'line_thickness': 2}
    def_patch_params = {'use_padding': True, 'contour_fn': 'center'}
    ...
    # NOTE: blocky step_size should NOT be patch_size if you want overlap; use patch_size if you want no-overlap blockmap.
    # You requested only skip-if-missing changes, so leaving your original behavior (step_size=patch_size) unchanged.
    blocky_wsi_kwargs = {
        'top_left': None,
        'bot_right': None,
        'patch_size': patch_size,
        'step_size': patch_size,  # original behavior
    ...
    for i in tqdm(range(len(process_stack))):
        ...
        # =========================
        # FIX #1: Create h5 if missing, and SKIP if it wasn't created
        # =========================
        if not os.path.isfile(h5_path):
            _ = compute_from_patches(
            ...
            # compute_from_patches may produce 0 patches => no h5 created
            if not os.path.isfile(h5_path):
                print(f"WARNING: h5 not created for {slide_id} -> likely 0 valid patches. Skipping ROI/slide.")
                try:
        ...
        # =========================
        # FIX #2: Create pt if missing (only after confirming h5 exists)
        # =========================
        ...
        if not os.path.isfile(block_map_save_path):
            ...
            _ = save_hdf5(block_map_save_path, asset_dict, mode='w')
        ...
        with h5py.File(block_map_save_path, 'r') as f:
        ...
        for sample in samples:
            if sample['sample']:
                ...
                for idx, (s_coord, s_score) in enumerate(zip(sample_results['sampled_coords'], sample_results['sampled_scores'])):
                    ...
                    patch.save(os.path.join(sample_save_dir, '{}_{}_x_{}_y_{}_a_{:.3f}.png'.format(
        ...
        # =========================
        # FIX #3: Heatmap pass - skip if save_path not created
        # =========================
        if heatmap_args.calc_heatmap:
            ...
            if not os.path.isfile(save_path):
                print(f"WARNING: heatmap h5 not created for {slide_id} (0 patches). Skipping visualization.")
                continue
        ...
        heatmap_save_name = '{}_{}_roi_{}_blur_{}_rs_{}_bc_{}_a_{}_l_{}_bi_{}_{}.{}'.format(
            ...
            int(heatmap_args.binarize), float(heatmap_args.binary_thresh),
        ...
        if heatmap_args.save_orig:
            ...
            orig_name = '{}_orig_{}.{}'.format(slide_id, int(vis_level), heatmap_args.save_ext)
            if not os.path.isfile(os.path.join(p_slide_save_dir, orig_name)):
                orig = wsi_object.visWSI(vis_level=vis_level, view_slide_only=True, custom_downsample=heatmap_args.custom_downsample)
                if heatmap_args.save_ext == 'jpg':
                    orig.save(os.path.join(p_slide_save_dir, orig_name), quality=100)
                else:
                    orig.save(os.path.join(p_slide_save_dir, orig_name))
