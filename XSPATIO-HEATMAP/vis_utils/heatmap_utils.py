            ...
            if attn_save_path is not None:
                ...
                if ref_scores is not None:
                    for score_idx in range(len(A)):
                        percentile_score = score2percentile(A[score_idx], ref_scores)
                        # Match the shape correctly
                        if np.isscalar(percentile_score):
                            A[score_idx] = np.array([percentile_score])
                        else:
                            A[score_idx] = np.mean(percentile_score)  # Reduce to a scalar or correct dimension
