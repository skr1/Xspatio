...
def get_contour_check_fn(contour_fn='center', cont=None, ref_patch_size=None, center_shift=None):
...
class Wsi_Region(Dataset):
    ...
    def __init__(
        ...
        for cont_idx, contour in enumerate(wsi_object.contours_tissue):
            print(f'processing {cont_idx}/{len(wsi_object.contours_tissue)} contours')
            ...
            # coord_results is typically a dict-like object with key 'coords'
            if coord_results is not None and len(coord_results) > 0 and 'coords' in coord_results:
                if len(coord_results['coords']) > 0:
        ...
        if len(filtered_coords) == 0:
            ...
            print(
                "WARNING: 0 coords after contour processing. "
                f"contours_tissue={len(wsi_object.contours_tissue)} "
                f"level={level} patch_size={patch_size} step_size={step_size} "
                f"top_left={top_left} bot_right={bot_right} contour_fn={contour_fn}"
            )
        else:
